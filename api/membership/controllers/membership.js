...
async function ApplyCode(receiver, price, code, plan) {
  ...
  if (
    ...
    affiliate !== null &&
    (affiliate.applied_for === "membership" ||
      ...
      if (affiliate.type === "limited") {
        ...
        if (limtedUsers.includes(parseInt(receiver))) {
          ...
          return {
            applied: false,
            codeApplied: referralCode,
            msg: "Affiliate type is limited, please check the user privilege",
          };
        }
      } else {
        let usedHistory = await strapi
    ...
    promocode !== null &&
    (promocode.applied_for === "membership" || promocode.applied_for === "both")
  ) {
...
module.exports = {
  ...
  async QRforExistingUser(reset = false) {
    if (reset === true) {
      var membershipWithNoQRcode = await strapi
        .query("membership")
        .find({ qrcode_url_ne: null });
    } else {
      var membershipWithNoQRcode = await strapi
        .query("membership")
        .find({ qrcode_url_null: true });
    }
    if (membershipWithNoQRcode.length > 0) {
      await Promise.all(
        membershipWithNoQRcode.map(async (member) => {
          if (member.user) {
            var qr = await createQRForExisting(member);
            await strapi
              .query("membership")
              .update({ id: member.id }, { qrcode_url: qr });
          } else {
            var qr = null;
          }
        })
  ...
  async generateMembership(ctx) {
    ...
    if (
      ...
      offerLimit = process.env.membershipPlanOfferLimit
        ? process.env.membershipPlanOfferLimit
        : 2000;
      totalOfferLimit = parseInt(offerLimit) + parseInt(checkUserExist.limit);
    }
    ...
    if (checkUserExist === null) {
      ...
      if (typeof params.type !== "undefined" && params.type === "manual") {
        ...
        var paidAmount = afterCodeApply.discountedPrice
          ? afterCodeApply.discountedPrice
          : 0;
      }

      let expiryDate = new Date(
        new Date().setMonth(new Date().getMonth() + packageSelected.duration)
      );
      ...
      await strapi.query("membership-transactions").create({
      ...
      if (checkUserExist !== null && checkUserExist.expiry !== null) {
        var expiryDate = new Date(
          new Date(checkUserExist.expiry).setMonth(
            new Date(checkUserExist.expiry).getMonth() +
              packageSelected.duration
          )
        );
      } else {
        var expiryDate = new Date(
      ...
      await strapi.query("membership-transactions").create({
        ...
        serial: serial,
        type: process.env.membershipRenewal
          ? process.env.membershipRenewal
          : "Renewal",
        expiry: expiryDate,
        amount: packageSelected.price,
        promocode_applied: afterCodeApply.applied === true ? code : null,
