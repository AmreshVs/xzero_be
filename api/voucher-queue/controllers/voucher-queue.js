...
module.exports = {
  async VoucherQueue(ctx) {
    ...
    let voucherDetails = await strapi
      .query("vouchers")
      .findOne({ id: voucher, status: true });
  ...
  async QueueCheck(user, voucher) {
    let voucherAvailed = await strapi
      ...
      .count({ user: user, voucher: voucher });
    if (!voucherAvailed) {
      await strapi
        .query("voucher-queue")
        .delete({ user: user, voucher: voucher });
    } else {
      let appBasicInfo = await strapi.query("app-basic-information").findOne();
      let voucherOnQueue = await strapi
        .query("voucher-queue")
        .findOne({ user: user, voucher: voucher });
      ...
      if (voucherOnQueue) {
        ...
        if (seconds > appBasicInfo.voucher_queue_expiry_seconds) {
          await strapi
            .query("voucher-queue")
            .update({ user: user, voucher: voucher }, { user_status: "idle" });
        }
      }
    }
