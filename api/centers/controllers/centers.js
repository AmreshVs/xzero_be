...
module.exports = {
  ...
  async top_centers(condition) {
    ...
    let offersByCenter = await Promise.all(centers.map(async ({ id, title_en, title_ar, place, city, featured_img }) => {
      let offersCount = await strapi.query('offers').count({ center: id });
      let discount = await strapi.query('offers').find({ center: id, _limit: 1, _sort: 'discount:desc' });
      ...
      return Promise.resolve({
        ...
        discount: await discount[0] !== undefined ? discount[0].discount : 0
      });
    }));
