...
module.exports = {
  async AddAsFavourite(user, offer, center = null) {
    ...
    if (center === null) {
      let offerDetails = await strapi
        .query("offers")
        .findOne({ id: offer, status: true });
      center = offerDetails.center.id;
    }
    //checking user is exist or not
    let favouriteExist = await strapi
      .query("favourites")
      .findOne({ user: user, center: center, status: true });
    let userFavourites = "";
    ...
    if (favouriteExist !== null) {
      await strapi.query("favourites").update(
        { user: user, center: center },
        {
          favourites:
            newFavourites.length > 1
              ? newFavourites.join(",")
              : pop === false || newFavourites.length === 1
              ? newFavourites[0]
              : null,
  ...
  //function will clear all added favourites
  async ClearAllFavourites(user) {
    let clearedFav = await strapi
      .query("favourites")
      .model.query((qb) => {
        qb.where({ user: user, status: true });
      })
      .save({ favourites: null }, { patch: true })
      .then(function (x) {
        return true;
      });
    if (clearedFav === true) {
