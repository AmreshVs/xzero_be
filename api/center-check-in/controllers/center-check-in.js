...
module.exports = {
        ...
        if(user_id !== null && center_id !== null) {
            ...
            if(offerArray.length<=membership.limit) {
                for (let index = 0; index < offerArray.length; index++) {
                    ...
                    let offersavailable = await strapi.query('offers').findOne({ id: offerArray[index] });
                    centeradd = await strapi.query('center-check-in').create({user_id:user_id, center_id:center_id, offer_id: offerArray[index], transaction_id:trid, discounted_price: offersavailable.discounted_price, original_price :offersavailable.actual_price, offer: offersavailable.discount});
                ...
                console.log(`You have chosen offers which exceeds the limit. Your limit is ${limit}, To add more please renew the membership or`)
            }
        } else {
    ...
    async getMembershipInfo( user_id, condition) {
        let offers = await strapi.query('offers').find({center:condition.center});
        let memberships = await strapi.query('membership').findOne({user: user_id});
