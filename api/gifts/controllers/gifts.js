...
module.exports = {
    ...
    if (user_id) {
      ...
      if (memberArray !== null && giftArray !== null) {
        ...
        if (
          ...
          // when the gift got as membership plan
          if (
            giftGotDetails.name_en.toLowerCase().replace(/\s/g, "").trim() ===
              String("membership1year") ||
            giftGotDetails.name_en.toLowerCase().replace(/\s/g, "").trim() ===
              String("membershiponeyear")
          ) {
            let membership = await strapi.services.membership.generateMembership(
              user_id,
              memberArray.package.id,
              12
            );
            if (membership) {
              ...
              var remarks =
                "got gift as membership 6 months plan and renewal failed";
            }
          } else if (
            giftGotDetails.name_en.toLowerCase().replace(/\s/g, "").trim() ===
              String("membership6months") ||
            giftGotDetails.name_en.toLowerCase().replace(/\s/g, "").trim() ===
              String("membershipsixmonths")
          ) {
            let membership = await strapi.services.membership.generateMembership(
              user_id,
              memberArray.package.id,
            ...
            if (membership) {
              ...
              var remarks =
                "got gift as membership 1 year plan and renewal failed";
            }
          }
          // code ends
  ...
  async AvailableGifts(condtion) {
    let gifts = [];
    if (
      typeof condtion.membership_plan !== "undefined" ||
      condtion.membership_plan !== null
    ) {
      gifts = await strapi.query("gifts").find({
        status: true,
        membership_plans: condtion.membership_plan,
        _limit: -1,
      });
    }
